...
public sealed class NTMenuConfigurationAnalyzer : DiagnosticAnalyzer {
    ...
    public override void Initialize(AnalysisContext context) {
        ...
        context.RegisterCompilationStartAction(static startContext => {
            ...
            var contextMenuType = startContext.Compilation.GetTypeByMetadataName("NTComponents.NTContextMenu");
            ...
            var componentTypes = new ComponentTypes(menuType, contextMenuType, buttonItemType, anchorItemType, dividerItemType, labelItemType, subMenuItemType);
    ...
    private static void AnalyzeComponentFrame(
        ...
        switch (frame.Kind) {
            case ComponentKind.Menu:
                AnalyzeMenuFrame(context, frame, componentTypes, colorType, "ChildContent");
                break;

            case ComponentKind.ContextMenu:
                AnalyzeMenuFrame(context, frame, componentTypes, colorType, "MenuContent");
                break;
    ...
    private static void AnalyzeMenuFrame(
        ...
        ComponentTypes componentTypes,
        INamedTypeSymbol colorType,
        string menuContentAttributeName) {
        ...
        AnalyzeMenuItems(context, frame, componentTypes, menuContentAttributeName);
    }
    ...
    private static void AnalyzeMenuItems(SyntaxNodeAnalysisContext context, ComponentFrame frame, ComponentTypes componentTypes, string menuContentAttributeName) {
        if (!frame.Attributes.TryGetValue(menuContentAttributeName, out var childContent)) {
    ...
    private static bool TryGetStaticMenuItemStatus(SyntaxNode? syntax, SemanticModel semanticModel, ComponentTypes componentTypes, out bool hasActionItem) {
        ...
        foreach (var invocation in invocations) {
            if (IsDynamicRenderFragmentContent(invocation, semanticModel)) {
                return false;
            }
    ...
    private static bool IsDynamicRenderFragmentContent(InvocationExpressionSyntax invocation, SemanticModel semanticModel) {
        if (!TryGetInvocationTarget(invocation, semanticModel, out var methodSymbol)
            || methodSymbol.Name != "AddContent"
            || methodSymbol.ContainingType.ToDisplayString() != "Microsoft.AspNetCore.Components.Rendering.RenderTreeBuilder"
            || invocation.ArgumentList.Arguments.Count < 2) {
            return false;
        }

        var contentOperation = UnwrapOperation(semanticModel.GetOperation(invocation.ArgumentList.Arguments[1].Expression));
        var contentTypeName = contentOperation?.Type?.OriginalDefinition.ToDisplayString();
        return contentTypeName is "Microsoft.AspNetCore.Components.RenderFragment"
            or "Microsoft.AspNetCore.Components.RenderFragment<TValue>";
    }
    ...
    private sealed class ComponentTypes(
        ...
        public ComponentKind GetComponentKind(ITypeSymbol componentType) {
            ...
            if (SymbolEqualityComparer.Default.Equals(componentType, contextMenu)) {
                return ComponentKind.ContextMenu;
            }
