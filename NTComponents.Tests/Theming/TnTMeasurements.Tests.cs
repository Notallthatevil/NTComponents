...
public class TnTMeasurements_Tests : BunitContext {
    ...
    public void BothStyleElements_AreRendered() {
        ...
        var scrollbarStyles = cut.FindAll("style.nt-scrollbars");

        measurementStyles.Should().HaveCount(1);
        fontStyles.Should().HaveCount(1);
        scrollbarStyles.Should().HaveCount(1);
    }
    ...
    [Fact]
    public void NtScrollbarClass_IsRendered() {
        // Arrange & Act
        var cut = RenderMeasurements();

        // Assert
        cut.Markup.Should().Contain(".nt-scrollbar{scrollbar-color:var(--tnt-color-primary-fixed) transparent;scrollbar-gutter:auto;scrollbar-width:thin;}");
        cut.Markup.Should().Contain(".nt-scrollbar::-webkit-scrollbar{block-size:8px;inline-size:8px;}");
        cut.Markup.Should().Contain(".nt-scrollbar::-webkit-scrollbar-thumb{background:var(--tnt-color-primary-fixed);background-clip:content-box;border:2px solid transparent;border-radius:9999px;}");
    }
