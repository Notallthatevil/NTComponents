...
public class NTCarousel_Tests : BunitContext {
    ...
    public void AdditionalAttributes_Class_Merged() {
        ...
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.AdditionalAttributes, attrs).AddChildContent(items));
    ...
    public void AdditionalAttributes_Style_Merged() {
        ...
        var items = BuildItems((1, "One", false, true, null));
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.BackgroundColor, TnTColor.Primary).Add(c => c.AdditionalAttributes, attrs).AddChildContent(items));
    ...
    public void AutoPlayInterval_And_Dragging_Attributes_Render() {
        ...
        var items = BuildItems((1, "One", false, true, null));
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.AutoPlayInterval, 5000).Add(c => c.AllowDragging, false).AddChildContent(items));
    ...
    public void BackgroundColor_Adds_Style_Variable() {
        ...
        var items = BuildItems((1, "One", false, true, null));
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.BackgroundColor, TnTColor.Primary).AddChildContent(items));
        // Assert
    ...
    public void CenterAlignedHero_With_Snapping_Adds_Centered_And_Snapping_Classes() {
        ...
        var items = BuildItems((1, "One", false, true, null));
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.Appearance, CarouselAppearance.CenterAlignedHero).Add(c => c.EnableSnapping, true).AddChildContent(items));
        var cls = cut.Find("tnt-carousel").GetAttribute("class")!;
        // Assert
    ...
    public void CenterAlignedHero_Without_Snapping_Does_Not_Add_Centered_Class() {
        ...
        var items = BuildItems((1, "One", false, true, null));
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.Appearance, CarouselAppearance.CenterAlignedHero).Add(c => c.EnableSnapping, false).AddChildContent(items));
        // Assert
    ...
    public void Hero_Appearance_Adds_Hero_Class_To_Root_And_Items() {
        ...
        var items = BuildItems((1, "One", false, true, null));
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.Appearance, CarouselAppearance.Hero).AddChildContent(items));
    ...
    public void M3_Appearance_Renders_Layout_Attribute(CarouselAppearance appearance) {
        ...
        var items = BuildItems((1, "One", false, true, null));
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.Appearance, appearance).AddChildContent(items));
        // Assert
    ...
    public void Item_OnClick_Invokes_Callback() {
        ...
        RenderFragment child = b => {
            ...
            b.AddAttribute(2, nameof(NTCarouselItem.OnClickCallback), EventCallback.Factory.Create(this, () => clicked++));
            b.AddAttribute(3, nameof(NTCarouselItem.ChildContent), (RenderFragment)(c => c.AddContent(0, "One")));
            b.CloseComponent();
        };
        // Act
        var cut = Render<NTCarousel>(p => p.AddChildContent(child));
    ...
    public void Removing_Item_Rerenders_List() {
        ...
        // Act
        var firstRender = Render<NTCarousel>(p => p.AddChildContent(firstItems));
        var secondRender = Render<NTCarousel>(p => p.AddChildContent(secondItems));
        // Assert
    ...
    public void Snapping_False_Removes_Snapping_Class() {
        ...
        var items = BuildItems((1, "One", false, true, null));
        // Act
        var cut = Render<NTCarousel>(p => p.Add(c => c.EnableSnapping, false).AddChildContent(items));
        // Assert
    ...
    private RenderFragment BuildItems(params (int order, string text, bool clickable, bool enableRipple, Dictionary<string, object>? additional)[] items) => b => {
        foreach (var (order, text, clickable, enableRipple, additional) in items) {
            b.OpenComponent<NTCarouselItem>(0);
            b.AddAttribute(10, nameof(NTCarouselItem.Order), order);
            if (clickable) {
                b.AddAttribute(20, nameof(NTCarouselItem.OnClickCallback), EventCallback.Factory.Create(this, () => { }));
            }
            b.AddAttribute(30, nameof(NTCarouselItem.EnableRipple), enableRipple);
            if (additional is not null) {
                b.AddAttribute(40, nameof(NTCarouselItem.AdditionalAttributes), additional);
            }

            b.AddAttribute(50, nameof(NTCarouselItem.ChildContent), (RenderFragment)(c => c.AddContent(0, text)));
            b.CloseComponent();
        }
    };
}
