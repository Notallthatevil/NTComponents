...
/// <summary>
///     Renders the day time-grid surface for <see cref="NTScheduler{TEventType}" />.
/// </summary>
/// <typeparam name="TEventType">The event type displayed by the scheduler.</typeparam>
public partial class NTSchedulerDayView<TEventType> where TEventType : TnTEvent {
    ...
    /// <summary>
    ///     The visible day column.
    /// </summary>
    [Parameter, EditorRequired]
    public NTSchedulerDayColumn<TEventType> Column { get; set; } = default!;
    ...
    private string GetColumnClass() {
        return CssClassBuilder.Create("nt-scheduler-day-view__day-column")
            .AddClass("nt-scheduler-day-view__day-column--today", Column.IsToday)
            .Build() ?? string.Empty;
    }
    ...
    private string GetEventStyle(NTSchedulerTimedEventLayout<TEventType> layout) {
        ...
        return string.Create(CultureInfo.InvariantCulture, $"top:{topPercent:0.##}%;height:{heightPercent:0.##}%;left:{leftPercent:0.##}%;width:calc({widthPercent:0.##}% - 8px);");
    }
