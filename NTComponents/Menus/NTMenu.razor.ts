...
export class NTMenu extends HTMLElement {
    ...
    private pointSourceElement: HTMLElement | null = null;

    private readonly onAnchorActivate = (): void => {
        this.clearPointAnchor();
    ...
    public openAt(clientX: number, clientY: number, sourceElement?: Element | null): void {
        this.pointAnchor = { x: clientX, y: clientY };
        this.pointSourceElement = sourceElement instanceof HTMLElement ? sourceElement : null;
        this.updatePlacement();
    ...
    private clearPointAnchor(): void {
        this.pointAnchor = null;
        this.pointSourceElement = null;
    }
    ...
    private handleKeyDown(event: KeyboardEvent): void {
        ...
        switch (event.key) {
            ...
            case 'Escape':
                event.preventDefault();
                this.hidePopover?.();
                this.getFocusReturnElement()?.focus?.();
                break;
            case 'ArrowRight':
    ...
    private updatePointPlacement(): void {
        const point = this.pointAnchor;
        if (!point) {
            return;
        }
        ...
        const viewportHeight = window.innerHeight || document.documentElement.clientHeight;
        const edgeMargin = 8;
        const spaceBelow = viewportHeight - point.y - edgeMargin;
        const spaceAbove = point.y - edgeMargin;
        const openTop = spaceBelow < menuSize.height && spaceAbove > spaceBelow;
        const spaceRight = viewportWidth - point.x - edgeMargin;
        const spaceLeft = point.x - edgeMargin;
        const alignStart = spaceRight >= menuSize.width || spaceRight >= spaceLeft;
        const top = openTop ? point.y - menuSize.height : point.y;
        const left = alignStart ? point.x : point.x - menuSize.width;
    ...
    private getFocusReturnElement(): HTMLElement | null {
        return this.pointSourceElement ?? this.getAnchorElement();
    }
