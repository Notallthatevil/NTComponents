...
/// <remarks>
///     <para>
///         Do use <see cref="NTDialog" /> for short, modal tasks that require a user decision, confirmation, focused content review, or a small amount of form input before returning to the current
///         workflow. Keep dialog copy concise and make the primary action clear.
///     </para>
///     <para>
///         Do place the dialog near the page or layout content that owns it and open it with a component reference, native <c>command</c>/<c>commandfor</c> attributes, or JavaScript by id. The dialog
///         content is ordinary Razor child content, so callers do not need to register root components or use a dialog service for owned dialog content.
///     </para>
///     <para>
///         Do provide a meaningful <see cref="Title" /> for assistive technology and keep action buttons in <see cref="Buttons" />. Use the optional <see cref="Icon" /> only when it adds quick
///         recognition; icon dialogs are center-aligned by design.
///     </para>
///     <para>
///         Do not use dialogs for nonblocking status, long reading experiences, broad navigation, or workflows that should remain visible beside the page. Prefer inline content, a page, a sheet, or
///         another persistent surface when users need background context while working.
///     </para>
///     <para>
///         By default, Escape can request closure and backdrop clicks do not close the dialog. Use <see cref="OnOpen" /> and <see cref="OnClose" /> to cancel lifecycle transitions when validation,
...
public partial class NTDialog {
    ...
    /// </summary>
    /// <remarks>
    ///     Keep dialog actions short, place the dismissive action before the confirming action, and use native <c>command="request-close"</c> with <c>commandfor</c> when possible so JavaScript can
    ///     coordinate the cancelable close lifecycle.
    /// </remarks>
    [Parameter]
    ...
    /// </summary>
    /// <remarks>
    ///     Defaults to <see langword="false" /> so accidental outside clicks do not dismiss modal work. Enable only for lightweight dialogs where losing the current dialog state is acceptable.
    /// </remarks>
    [Parameter]
    ...
    /// </summary>
    /// <remarks>
    ///     Defaults to <see langword="true" /> for native modal keyboard behavior. Use <see cref="OnClose" /> when Escape should be conditionally blocked by validation or unsaved changes.
    /// </remarks>
    [Parameter]
    public bool CloseOnEscape { get; set; } = true;

    /// <summary>
    ///     Gets or sets the dialog element id. This is an alias for component-style usage; the base component element id remains supported.
    /// </summary>
    [Parameter]
    ...
    /// </summary>
    /// <remarks>
    ///     Defaults to <see cref="NTElevation.Medium" /> to keep the modal container visually separated from the scrimmed page surface. Lower it only when the surrounding surface hierarchy already
    ...
    /// </summary>
    /// <remarks>
    ///     Icons are decorative and rendered with <c>aria-hidden</c>. Use an icon for quick recognition of a dialog category or outcome, not as the only way to communicate severity or meaning.
    /// </remarks>
    [Parameter]
    ...
    /// <summary>
    ///     Invoked before an open request continues. Set <see cref="NTDialogEventArgs.Cancel" /> to <see langword="true" /> to stop the dialog from opening.
    /// </summary>
    /// <remarks>
    ///     Use this for guards that decide whether a requested dialog should open. Keep work short because the native open is waiting for this callback.
    /// </remarks>
    [Parameter]
    ...
    /// <summary>
    ///     Invoked before a close request continues. Set <see cref="NTDialogEventArgs.Cancel" /> to <see langword="true" /> to stop the dialog from closing.
    /// </summary>
    /// <remarks>
    ///     Use this for validation, dirty-state confirmation, or policy checks that must prevent closure. If the close is allowed, <see cref="OnClosing" /> and <see cref="OnClosed" /> continue the
    ///     lifecycle.
    /// </remarks>
    [Parameter]
    ...
    /// </summary>
    /// <remarks>
    ///     Keep supporting text brief. If the explanation needs multiple paragraphs, prefer putting that content in <see cref="ChildContent" /> so the body scroll and divider behavior applies.
    /// </remarks>
    [Parameter]
    ...
    /// </summary>
    /// <remarks>
    ///     Provide a concise title that names the decision or task. The title is used as the dialog label through <c>aria-labelledby</c>.
    /// </remarks>
    [Parameter]
